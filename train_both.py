...
def train_epoch(
    ...
    model_t = module_dict["teacher"].train()#eval()

    policy = module_dict["policy"].train()

    hints = []
    anti_hints = []
    des_middle_layer = layers  # [0, 1, 2, 3, 4, 100]
    # print(des_middle_layer)
    ...
    for idx, data in enumerate(train_loader):
        ...
        middle_layer = [des_middle_layer[i] if des_middle_layer[i] > 0 else min(len(feat_s), len(feat_t)) + des_middle_layer[i] for
                        i in range(len(des_middle_layer))]
        ...
        if cfg["kd_loss"]["name"] == "FitNet":
            ...
            g_t = [g_t[feat][middle_choose_list[feat] > 0.5] if
        ...
        if logit_s[choose_index].shape[0] > 0:
            loss_div += (criterion_div(logit_s[choose_index], logit_t[choose_index]) * logit_s[choose_index].shape[0] / logit_s.shape[0])

        loss_ori_cls = gamma * ( criterion_cls(logit_s, target) + criterion_cls(logit_t, target))
        loss_cls = gamma * loss_cls
        ...
        top1_t.update(acc1_t[0], x.shape[0])
        top5_t.update(acc5_t[0], x.shape[0])
        ...
        if idx % cfg["training"]["print_iter_freq"] == 0:
            if len(ac_middle) == 1:
                logger.info("graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, middle cnt: %d",
                            loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                            logit_t[ac_middle[0] > 0.5].shape[0])
            elif len(ac_middle) == 2:
                logger.info(
                    "graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, cnt1: %d, cnt2: %d",
                    loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                    logit_t[ac_middle[0] > 0.5].shape[0], logit_t[ac_middle[1] > 0.5].shape[0])
            elif len(ac_middle) == 3:
                logger.info(
                    "graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, cnt1: %d, cnt2: %d, cnt3: %d",
                    loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                    logit_t[ac_middle[0] > 0.5].shape[0], logit_t[ac_middle[1] > 0.5].shape[0],
                    logit_t[ac_middle[2] > 0.5].shape[0])
            elif len(ac_middle) == 4:
                logger.info(
                    "graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, cnt1: %d, cnt2: %d, cnt3: %d, cnt4: %d",
                    loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                    logit_t[ac_middle[0] > 0.5].shape[0], logit_t[ac_middle[1] > 0.5].shape[0],
                    logit_t[ac_middle[2] > 0.5].shape[0], logit_t[ac_middle[3] > 0.5].shape[0])
            elif len(ac_middle) == 5:
                logger.info(
                    "graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, cnt1: %d, cnt2: %d, cnt3: %d, cnt4: %d, cnt5: %d",
                    loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                    logit_t[ac_middle[0] > 0.5].shape[0], logit_t[ac_middle[1] > 0.5].shape[0],
                    logit_t[ac_middle[2] > 0.5].shape[0], logit_t[ac_middle[3] > 0.5].shape[0],
                    logit_t[ac_middle[4] > 0.5].shape[0])
            elif len(ac_middle) == 6:
                logger.info(
                    "graftloss_cls: %.5f, loss_cls: %.5f, loss_div: %.5f, loss_kd: %.5f, cnt1: %d, cnt2: %d, cnt3: %d, cnt4: %d, cnt5: %d, cnt6: %d",
                    loss_cls.item(), loss_ori_cls.item(), loss_div.item(), loss_kd.item(),
                    logit_t[ac_middle[0] > 0.5].shape[0], logit_t[ac_middle[1] > 0.5].shape[0],
                    logit_t[ac_middle[2] > 0.5].shape[0], logit_t[ac_middle[3] > 0.5].shape[0],
                    logit_t[ac_middle[4] > 0.5].shape[0], logit_t[ac_middle[5] > 0.5].shape[0])

            logger.info(
                "Epoch: [%3d|%3d], idx: %d, total iter: %d, loss: %.5f, acc@1: %.4f, acc@5: %.4f, acc_t@1: %.4f, acc_t@5: %.4f",
                epoch, cfg["training"]["epochs"],
                idx, __global_values__["it"],
                losses.val, top1.val, top5.val,
                top1_t.val, top5_t.val
            )

    ret_cnts = [each_cnt.avg for each_cnt in cnts]

    return top1.avg, losses.avg, ret_cnts
...
def train_kd(
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        train_acc, train_loss, re_cnts = train_epoch(
            cfg=cfg,
            epoch=epoch,
        ...
        tb_writer.add_scalar("epoch/cnt0", re_cnts[0], epoch)
        tb_writer.add_scalar("epoch/cnt1", re_cnts[1], epoch)
        tb_writer.add_scalar("epoch/cnt2", re_cnts[2], epoch)
        tb_writer.add_scalar("epoch/cnt3", re_cnts[3], epoch)
        tb_writer.add_scalar("epoch/cnt4", re_cnts[4], epoch)
        ...
        val_acc_t, val_acc_top5_t, _ = validate(
            val_loader=val_loader,
        ...
        logger.info(
            "Epoch: %04d | %04d, acc: %.4f, loss: %.5f, val_acc: %.4f, val_acc_top5: %.4f, val_loss: %.5f, val_acc_t: %.4f, val_acc_top5_t: %.4f",
            epoch, cfg["training"]["epochs"],
            train_acc, train_loss,
            val_acc, val_acc_top5, val_loss,
            val_acc_t, val_acc_top5_t
        )
...
def main(
    ...
    if cfg["kd_loss"]["name"] in ['AT', 'NST', 'VID', 'KDSVD', 'AB']:
        ...
        # print(len(feat_t_lis), len(feat_s_lis),real_len)
        ...
        if True:  # not cfg["kd_loss"]["homo"]:
            for each_feat in range(real_len):
                print(feat_s_lis[each_feat].shape, feat_t_lis[each_feat].shape)
                module_dict["hint_{}".format(each_feat)] = ConvReg(feat_s_lis[each_feat].shape,
                                                                   feat_t_lis[each_feat].shape).to(device)
                module_dict["anti_hint_{}".format(each_feat)] = ConvReg(feat_t_lis[each_feat].shape,
                                                                        feat_s_lis[each_feat].shape).to(device)
    ...
    # print("use hint: ", not cfg["kd_loss"]["homo"])
